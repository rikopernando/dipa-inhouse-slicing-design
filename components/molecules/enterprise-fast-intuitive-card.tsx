...
interface EnterpriseFastIntuitiveCardProps {
  card: EnterpriseCardType;
}

/**
 * EnterpriseFastIntuitiveCard Component
 *
 * Displays an enterprise feature card with image and description
 * Part of the Enterprise Section organism
 */
export function EnterpriseFastIntuitiveCard({ card }: EnterpriseFastIntuitiveCardProps) {
  return (
    <div
      className={cn(
        'group bg-enterprice-card relative flex h-[400px] flex-col overflow-hidden rounded-3xl px-8 pt-8 transition-shadow duration-300 hover:shadow-lg',
        card.colSpan,
      )}
    >
      {/* Card Content */}
